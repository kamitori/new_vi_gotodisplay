...
class ProductsController extends AdminController {
	...
	public function listProduct()
	{
		...
		$products = Product::with('mainImage')->with('categories')->with('layout')->with('type')
							->select(DB::raw('id, name,svg_layout_id, sku,product_type_id, sell_price, short_description,svg_file, active,
												(SELECT COUNT(*)
													FROM notifications
		...
		if(!empty($arrProducts)){
			foreach($arrProducts as $product){
				if( isset($product->main_image[0]) ) {
					$image = URL.'/'.$product->main_image[0]->path;// DOI FIX XONG BUG CHOMOD ROI MO LAI // URL.'/'.str_replace('/images/products', '/images/products/thumbs', $product->main_image[0]->path);
				} else {
				...
				if(isset($product->categories)){
					$arr_category = $product->categories;
					foreach($arr_category as $ttt){
						$v_cagegory_name = $ttt->name;
					}					
				}				
				if(isset($product->svg_file) && $product->svg_file!='' ){
					$v_svg_file = '/'.$product->svg_file;
				}else{					
					if(isset($product->layout)){
						$arr_layout = $product->layout;
						$v_svg_file = '/'.$arr_layout->svg_file;
					}
				}
				if(!file_exists(public_path().$v_svg_file) || $v_svg_file=='' || is_null($v_svg_file)){
					$v_svg_file = '/assets/images/noimage/247x185.gif';
				}
				if(isset($product->type)){
					$arr_type = $product->type;
					$v_product_type = $arr_type->name;
				}
		...
		if( $pass->passes() ) {
			...
			if( Input::has('images') ) {
				...
				foreach($images as $key => $image) {
					...
					if( isset($image['main']) ) {
						settype($image['main'], 'int');
						$data['main'] = $image['main'] == 1 ? 1 : 0 ;
						$data['back'] = $image['main'] == 2 ? 1 : 0 ;
						$data['square'] = $image['main'] == 3 ? 1 : 0 ;
						$data['2d'] = $image['main'] == 4 ? 1 : 0 ;
					}
