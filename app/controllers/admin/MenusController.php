...
class MenusController extends AdminController {
	...
	public function index()
	{
		...
		$banners = Banner::select('id', 'name', 'order_no', 'active', 'type')->where('active', 1)
														// ->where('type', 'small banner')
														->with('images');		
		
		$arr_list_big_banner = array();
		$arr_list_small_banner = array();

		$arrBanners = $banners->get()->toArray();
		$arrSmallBanners = $arrBigBanners = array();	
		if(!empty($arrBanners)){
			foreach($arrBanners as $key => $banner){
				...
				$banner['image'] = $image;
				if($banner['type'] == 'small banner'){
					$arrSmallBanners[] = $banner;

					$temp = Menu::select('name','link')->where('small_banners','like','%'.$banner['id'].'%')->get();
					foreach($temp as $arr){
						$arr_list_small_banner [$arr['name']] = array(
							'name'=>$arr['name']
							,'link'=>($arr['link']=='' ? url() : url().'/'.$arr['link'])
						);
					}
				} else {
					$arrBigBanners[] = $banner;

					$temp = Menu::select('name','link')->where('big_banners','like','%'.$banner['id'].'%')->get();
					
					foreach($temp as $arr){
						$arr_list_big_banner [$arr['name']] = array(
							'name'=>$arr['name']
							,'link'=>($arr['link']=='' ? url() : url().'/'.$arr['link'])
						);
					}
        ...
        $this->layout->content = View::make('admin.menus-all')->with([
        																...
        																'arrSmallBanners' => $arrSmallBanners,
        																'arrBigBanners' => $arrBigBanners,
        																'arr_list_big_banner'=>$arr_list_big_banner,
        																'arr_list_small_banner'=>$arr_list_small_banner
        															]);
	}
	...
	public function updateBanner($type='')
	{
		if( Request::isMethod('get') ) {
        	return Redirect::to(URL.'/admin/menus');
		}
		...
		if(!empty($arrPost)){
			// pr($arrPost);exit;
			if(  $arrPost['mid'] ) {
            	$message = 'with banners have been updated successfully!';
				$menu = Menu::find($arrPost['mid']);

				if($type == 'big'){
					if(!empty($arrPost['banner-selected'])){
						$banners = implode(",", $arrPost['banner-selected']);
						$menu->big_banners 		= $banners;
					} else {
						$menu->big_banners 		= null;
					}
					
				} elseif($type == 'small') {
					if(!empty($arrPost['banner-selected'])){
						$banners = implode(",", $arrPost['banner-selected']);
						$menu->small_banners 		= $banners;
					} else {
						$menu->small_banners 		= null;
					}
