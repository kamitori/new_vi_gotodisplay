...
class AdminsController extends AdminController {
	...
	public function listAdmin()
	{
		...
		$admins = Admin::select('id', 'first_name', 'last_name', 'email', 'active')->with('images');
		if(!empty($search)){
	...
	public function updateAdmin()
	{
		...
		if( $pass->passes() ) {
			...
			if( Input::hasFile('image') ) {
				$imageId = VIImage::upload(Input::file('image'), public_path().DS.'assets'.DS.'images'.DS.'admins', 110, false);
	...
	public function imageBrowser($page = 1)
	{
		if( Request::ajax() ) {
			...
			$response = Response::json(VIImage::imageBrowser('admins', $page, false));
			$response->header('Content-Type', 'application/json');
			return $response;
		}
		return App::abort(404);
