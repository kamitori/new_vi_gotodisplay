<?php
class ShapeLayoutsController extends AdminController {

	public static $table = 'shape_layouts';
	...
	{
		$this->layout->title = 'Shape Layout';
	...
	public function listLayout()
	{
		...
		$layouts = ShapeLayout::select('id', 'name',  'active', 'svg_file');
		if(!empty($search)){
		...
		$arrReturn = ['draw' => Input::has('draw') ? Input::get('draw') : 1, 'recordsTotal' => ShapeLayout::count(), 'recordsFiltered' => $count, 'data' => []];
		if(!empty($arrLayouts)){
	...
	{
		try {
			$layout = ShapeLayout::with('shapes')
								->findorFail($layoutId);
	...
	public function updateLayout()
	{
		...
		if( Input::has('id') ) {
			$create = false;
			try {
				$layout = ShapeLayout::findorFail( (int)Input::get('id') );
			} catch(Illuminate\Database\Eloquent\ModelNotFoundException $e) {
				return App::abort(404);
			}
		...
		$fileName = md5($layout->name).'-shape.svg';
		...
		if( $pass ) {
			...
			if( Input::has('svg') ) {
				...
				foreach($svg as $shape) {
					$arr = [
						...
						'rotate'    => $shape['rotate'],
						'd'    => $shape['d'],
						'shape_type'=> $shape['shape-type'],
						'transform' => isset($shape['transform']) ? $shape['transform'] : ''
					];
					if( isset($shape['id']) && is_numeric($shape['id']) ) {
						if( isset($shape['deleted']) ) {
							$arrDelete[] = $shape['id'];
						} else {
							$layout->shapes()->where('shape_layout_details.id', $shape['id'])->update($arr);
						}
					} else {
						$arrInsert[] = new ShapeLayoutDetail($arr);
					}
				}

				if( !empty($arrDelete) ) {
					ShapeLayoutDetail::destroy($arrDelete);
				}
			...
			if( Input::has('continue') ) {
				if( $create ) {
					$prevURL = URL.'/admin/shape-layouts/edit-layout/'.$layout->id;
				}
				return Redirect::to($prevURL)->with('flash_success',"<b>$layout->name</b> {$message}.");
			}
			return Redirect::to(URL.'/admin/shape-layouts')->with('flash_success',"<b>$layout->name</b> {$message}.");
		}
	...
	public function deleteLayout($id)
	{
		if( Request::ajax() ) {
			...
			try {
				$layout = ShapeLayout::findorFail($id);
			} catch(Illuminate\Database\Eloquent\ModelNotFoundException $e) {
				return App::abort(404);
			}
			$name = $layout->name;
