<script type="text/javascript">
    ...
    $("#dsbt_my_upload").on('click',function(){
        $("#content_my_upload").css('display','block');
    ...
    function changeSize(sizes,price){ // sizes = 10x12
        ...
        if(sizes!='' && sizes.indexOf("x")!=-1){
            ...
            console.log('Can not found size');
        if($("#sizes_custom").is(':checked')) 
            price = caculationPrice(w,h,1, false);
        else
            price = $("input:radio[name=sizes]:checked").attr("rel");
    ...
    function changeBleed(thickness){
        ...
        rebuildFrame(w,h);
        name_thick = arr_bleeds[thickness];
        console.log(name_thick);
        $("#name_bleed").html(' - '+name_thick);
    ...
    function drawDesignSVG(objImg,objSize,objOrientation,objEdgeColor){
        ...
        @else
            ...
            $(".right_content").on("touchstart",function(e){
                if ($(".right_content")[0] == e.target) {
                    $('#zoom_bt').hide();
                    $('#image_bt').hide();
                    $(".slider_bt").hide();
        ...
        for( var i = 0; i < files.length; i++ ) {
            ...
            reader.onload = function(e){
                ...
                img.onload = function(){
                    ...
                    check_file = check_list_quality(width,height);
                    if(true){
                        ...
                        $.ajax({
                            ...
                            {                                
                                ...
                                // $("#loading_p_bar").css('width','100%');
                                // $("#loading_p_bar").html('100%');                                
                            }
                        });
                    }else{
                        ///
    ...
    function change_upload_image_one_image(result){
        ...
        @if( $product['product_type']==6 && ($product['number_img']==2 || $product['number_img']==3))
            var html = "<div class=box_image'> <img class='imgFocus photo img_upload ui-draggable' src='"+objImg.url+"' alt='title' onclick='changeImage(this);' /></div>";
            $("#content_my_upload img").removeClass('imgFocus');
        @else
            var html = "<div class='box_image' ><img  class='photo img_upload ui-draggable' src='"+objImg.url+"' alt='title' data-id="+ objImg.dataId +" onclick='changeImage(this);' /></div>";
            $("#svg_main").html('');
        @endif
        var _screen_width = parseInt($(document).width(),10);
        if(_screen_width<=640) html = '<div class="col-sm-6 col-xs-6" >' + html + '</div>';
    ...
    function check_list_quality(width,height){
        ...
        if (rating==1) {
            $(".check_quality").html(" Image Quality : Excellent");
            $(".check_quality").css("color", "#009900");
        } else if (rating==2) {
            $(".check_quality").html(" Image Quality : Good");
            $(".check_quality").css("color", "rgb(0,0,255)");
        } else if (rating==3) {
            $(".check_quality").html(" Image Quality : Fair");
            $(".check_quality").css("color", "#6600CC");
        } else {
            $(".check_quality").html(" Image Quality : Poor");
            $(".check_quality").css("color", "rgb(112,112,112)");
        }
