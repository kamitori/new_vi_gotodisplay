...
<script type="text/javascript">
	...
	var bg_back_image = $("#bg_back_image").val();
	var bg_square_image = $("#bg_square_image").val();
	function build3D(object, callBack){
		...
		@if($isMobile)
			...
			window_height = window_width*55/100;
        ...
        if(bg_back_image==''){
        	...
        	var size_ = $("input[name='sizes']:checked");
        	var _type = size_.attr('id');
        	var horizontal_image = $("#bg_horizontal_image").val();
        	var vertical_image = $("#bg_vertical_image").val();
        	var hori_ver_image = '';
        	if(_type=='sizes_custom'){
        		var _width = parseInt($("#custom_width").val(),10);
        		var _height = parseInt($("#custom_height").val(),10);

        		if(_width>_height){
        			hori_ver_image = horizontal_image;
        		}else if(_width<_height){
        			hori_ver_image = vertical_image;
        		}else{
        			if(bg_square_image!='') hori_ver_image = bg_square_image;
        		}

        	}else{
        		var _sizes = size_.val();
        		var _split = _sizes.split('x');
        		var _width = parseInt(_split[0],10);
        		var _height = parseInt(_split[1],10);
        		if(_width>_height){
        			hori_ver_image = horizontal_image;
        		}else if(_width<_height){
        			hori_ver_image = vertical_image;
        	...
        	if(hori_ver_image!='') bg_back_image = hori_ver_image;
        	back = new materialClass( 
            { 
                map: THREE.ImageUtils.loadTexture('/'+bg_back_image)
                , ambient: 0x123456
                , overdraw: 0.5 
            });
        }
		if( !object.hasOwnProperty("points") ) {
			...
			var is_wrap = $("#dsbt_style").is(':visible');
			
			if(bg_back_image=='' || is_wrap){
				var materials = [
					right,      // Right side
					left,       // Left side
					top,        // Top side
