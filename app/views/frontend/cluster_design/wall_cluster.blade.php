...
function drawDesignSVG(objImg,objSize,objOrientation,objEdgeColor){
    ...
    @foreach($svg_setup['data'] as $key=>$value)
        ...
        @if($isMobile)
        document.getElementById(image_{{$key}}.node.id).addEventListener('touchstart',chooseImage,false);
        @else
        document.getElementById(image_{{$key}}.node.id).addEventListener('click',chooseImage,false);
        @endif
        if(first_run==0)
