...
class Menu extends BaseModel {
	...
	{
		$arrMenu = self::select('id','name','icon_class','link','type', 'parent_id','group', 'order_no','level', 'active', 'big_banners', 'small_banners');
		...
		if( isset($arrMenu[$parent_id]) ){
			...
			foreach($arrMenu[$parent_id] as $k => $menu){
				...
				if ( isset($arrMenu[$id]) ) {
					...
					$arrHTML['mobile'] .= '<li class="nav-item dropdown tree-toggle nav-header">
				                            <a class="dropdown-link" href="#">
				                            	'. $menu['name'] .'
					                            <span aria-hidden="true" class="glyph plus"></span>
					                            <span aria-hidden="true" class="glyph minus"></span>
				                            </a>
				                            <ul class="sub-nav catalog nav nav-list tree">
				                                '. $children['mobile'] .'
				                            </ul>
					                    </li>';
				} else {
		...
		if( isset($arrMenu[$parent_id]) ){
			...
			foreach($arrMenu[$parent_id] as $k => $menu){
				...
				if ( isset($arrMenu[$id]) ) {
					...
					if( $parent_id ) {
						$html .= '<li>
									<a href="'.URL.'/'. $menu['link'] .'" title="'. $menu['name'] .'" style="text-transform:capitalize;">'. $menu['name'] .'</a>
								</li>';
					} else {
						$html .= '<div class="column-'. $i .' large-3 columns">
									<b class="title"><a href="'. $menu['name'] .'" title="'. $menu['name'] .'" /></b>
								</div>';
					}
				}
				unset($arrMenu[$parent_id][$k]);
