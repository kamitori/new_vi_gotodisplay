...
class Product extends BaseModel {
	...
	public static function getSmallestPrice($product, $includeSize = false)
	{
		...
		$smallestDepth = ProductOption::select('name','key')
		...
		if( is_object($smallestDepth) ) {
			$bleed = floatval($smallestDepth->key);
		}
		...
		if( !$products->isEmpty() ) {
			foreach($products as $key => $product) {
				...
				if( isset($product->images[0]) ) {
					$product->image = URL.'/'.$product->images[0]->path;//str_replace('/images/products/', '/images/products/thumbs/', $product->images[0]->path);
				} else {
