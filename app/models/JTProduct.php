...
class JTProduct extends JT {
	...
	public static function calculateCostPrice($product, $sizew, $sizeh)
	{
        ...
        if( isset($product['options']) ) {
        	foreach($product['options'] as $option){
        		...
        		if( !isset($option['require']) || !$option['require']) continue;
        		if( !isset($option['product_id']) || !is_object($option['product_id']) ) continue;
        		$tmpOpt = self::select('sku', 'code', 'name', 'sell_price','sell_by', 'priceBreak_2s', 'sellprices')
        					->where('_id', '=', new MongoId( $option['product_id']  ))
        					->where('deleted', '=', false)
        					->first()
        					->toArray();
        					
        		$tmpOpt['price_break'] = self::priceBreak_2($tmpOpt, $company);
        		...
        		$option['quantity'] = 1;
        		$option['price_break'] = self::priceBreak_2($option, $company);
		...
		$product['price_break'] = self::priceBreak_2($product, $company);
	...
	public static function companypriceBreak_2($company, $productId)
	{
	...
	public static function productpriceBreak_2($arrData, $sell_category_key = '')
	{
		$result = [];
		...
		//tim Price breaks
		if(isset($arrData['priceBreak_2s']) && is_array($arrData['priceBreak_2s']) && count($arrData['priceBreak_2s'])>0){
			foreach($arrData['priceBreak_2s'] as $key => $value){
				if( isset($value['deleted']) && $value['deleted'] ) continue;
				if( !isset($value['sell_category']) ) continue;
	...
	public static function priceBreak_2($arrData, $company = array())
	{
		$result = [];

		$sell_category = self::companypriceBreak_2($company,$arrData['_id']);
		...
		$sell_break = self::productpriceBreak_2($arrData, $sell_category['sell_category_key']);
	...
	public static function calpriceBreak_2(&$arrData){
	...
	public static function calSubTotal(&$arrData){
		if(isset($arrData['unit_price']) && $arrData['unit_price']!='' && isset($arrData['quantity'])){
			...
			if(isset($arrData['discount']) && intval($arrData['discount']) != 0) {
				$arrData['sub_total'] = $arrData['sub_total'] * (100 - (float)$arrData['discount']) / 100;
			}
		}
		else
			$arrData['sub_total'] = 0;
	...
	public static function getPrice($data)
	{
		...
		if( isset($data->sku) ) {
			...
			// pr($data->toArray());
            $options = [];
			$jt_product = self::select('_id','code', 'name', 'sku', 'sell_price', 'oum', 'oum_depend', 'sell_by','priceBreak_2s', 'sellprices', 'unit_price','options', 'products_upload', 'product_desciption', 'is_custom_size')
                                ->where('sku', $data->sku)
                                ->where('deleted', false)
                                ->first();
            if( is_object($jt_product) ) {
                ...
                $jt_product['price_break'] = self::priceBreak_2($jt_product, $company);
